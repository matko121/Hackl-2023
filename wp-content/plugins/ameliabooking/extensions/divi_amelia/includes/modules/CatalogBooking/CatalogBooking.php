...
class DIVI_CatalogBooking extends ET_Builder_Module
{
    ...
    public function get_fields()
    {
        $array = array(
            ...
            'categories' => array(
                ...
                'type'            => AMELIA_LITE_VERSION ? 'select' : 'amelia_multi_select',
    ...
    public function render($attrs, $content = null, $render_slug = null)
    {
        ...
        if ($catalog !== '0') {
            ...
            if ($category && !empty($category) && is_numeric($category[0]) && $catalog === 'category') {
                $shortcode .= ' category=' . implode(',', $category);
            } else if ($service && !empty($service) && is_numeric($service[0]) && $catalog === 'service') {
                $shortcode .= ' service=' . implode(',', $service);
            } else if ($package1 && !empty($package1) && is_numeric($package1[0]) && $catalog === 'package') {
        ...
        if ($preselect === 'on') {
            ...
            if ($employee && !empty($employee) && is_numeric($employee[0])) {
                $shortcode .= ' employee=' . implode(',', $employee);
            }
            if ($location && !empty($location) && is_numeric($location[0])) {
