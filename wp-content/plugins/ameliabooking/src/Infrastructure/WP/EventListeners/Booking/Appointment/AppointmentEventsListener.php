        ...
        if ($param->getResult() !== 'error') {
            ...
            switch ($event->getName()) {
                ...
                case 'AppointmentDeleted':
                    if (isset($param->getData()["event"]) &&
                        in_array($param->getData()["event"]["status"], ["canceled", "rejected", "deleted"])
                    ) {
                        return;
                    }
                ...
                case 'BookingEdited':
                    if (AMELIA_LITE_VERSION &&
                        isset($param->getData()["event"]) &&
                        in_array($param->getData()["booking"]["status"], ["canceled", "rejected", "deleted"])
                    ) {
                        return;
                    }
