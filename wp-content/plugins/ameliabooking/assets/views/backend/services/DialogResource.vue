  ...
  <div>
    <div class="am-dialog-scrollable" :class="{'am-edit':resource.id !== 0}">
      ...
      <el-form :model="resource" ref="resource" :rules="rules" label-position="top" @submit.prevent="onSubmit">
        ...
        <!-- Name -->
        <el-form-item prop="countAdditionalPeople">
          <el-row :gutter="16">
            <el-col :lg="20">
              {{ $root.labels.resource_additional_people }}:
              <el-tooltip placement="top">
                <div slot="content" v-html="$root.labels.resource_additional_people_tooltip"></div>
                <i class="el-icon-question am-tooltip-icon"></i>
              </el-tooltip>
            </el-col>
            <el-col :lg="4" style="text-align: center">
              <el-switch
                  v-model="resource.countAdditionalPeople"
                  @change="changeCountAdditionalPeople"
              >
              </el-switch>
            </el-col>
          </el-row>
        ...
        <el-form-item :label="$root.labels.resource_quantity + ':'">
          <el-input-number v-model="resource.quantity" :min="resource.countAdditionalPeople ? 2 : 1" auto-complete="off" @input="clearValidation()"></el-input-number>
        </el-form-item>
...
export default {
  ...
  methods: {
    changeCountAdditionalPeople (val) {
      if (val && this.resource.quantity < 2) {
        this.resource.quantity = 2
      }
    },
    ...
    getParsedEntity () {
      ...
      return {
        ...
        countAdditionalPeople: this.resource.countAdditionalPeople
      }
    },
