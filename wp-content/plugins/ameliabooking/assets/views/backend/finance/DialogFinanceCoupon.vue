      ...
      <el-form :model="coupon" ref="coupon" :rules="rules" label-position="top" @submit.prevent="onSubmit">
        ...
        <el-tabs v-model="couponTabs">
          ...
          <el-tab-pane :label="$root.labels.details" name="details">
            ...
            <!-- Packages -->
            <el-form-item label="placeholder" v-if="($root.licence.isPro || $root.licence.isDeveloper) && packages.length > 0" prop="packages">
              <label slot="label">
                {{ $root.labels.packages }}:
                <el-tooltip placement="top">
                  <div slot="content" v-html="$root.labels.packages_tooltip"></div>
                  <i class="el-icon-question am-tooltip-icon"></i>
                </el-tooltip>
              </label>
              <el-select
                  v-model="coupon.packageList"
                  value-key="id"
                  filterable
                  multiple
                  :placeholder="$root.labels.select_package"
                  collapse-tags
                  @change="clearValidation()"

              >
                <div class="am-drop-parent"
                     @click="allPackagesSelection"
                >
                  <span>{{ $root.labels.select_all_packages }}</span>
                </div>
                <el-option
  ...
  export default {
    ...
    data () {
      ...
      let validateEntitiesCount = (rule, value, callback) => {
        if (
          this.coupon.serviceList.length === 0 &&
            this.coupon.eventList.length === 0 &&
            ((this.$root.licence.isPro || this.$root.licence.isDeveloper) && this.coupon.packageList.length === 0)
        ) {
