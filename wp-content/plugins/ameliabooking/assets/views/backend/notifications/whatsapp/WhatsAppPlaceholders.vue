<template>
  ...
  <div class="am-whatsapp-placeholders">
    <el-alert
      v-if="name !== 'header' || (whatsappPlaceholders[0] && whatsappPlaceholders[0].type !== 'image')"
      type="warning"
      show-icon
      title=""
    ...
    <el-form ref="whatsappPlaceholders" :model="{whatsappPlaceholders: whatsappPlaceholders}" :rules="rules" @submit.prevent="onSubmit">
      <div v-for="(placeholder, index) in whatsappPlaceholders" :key="index">
        <el-row>
          <p v-if="placeholder.type !== 'image'" v-html="$root.labels.placeholder + ' {{' + (index+1) + '}}:'"/>
          <p v-else v-html="$root.labels.whatsapp_header_image"/>
        </el-row>
        <el-row type="flex" style="gap: 30px" v-if="placeholder.type !== 'image'">
          <el-col :span="12">
        ...
        <el-row type="flex" style="gap: 30px" v-else>
          <el-col>
            <el-form-item :prop="'whatsappPlaceholders.'+index+'.type'">
              <el-input
                  v-model="placeholder.value"
                  :placeholder="$root.labels.enter_image_link"
              />
            </el-form-item>
          </el-col>
        </el-row>
