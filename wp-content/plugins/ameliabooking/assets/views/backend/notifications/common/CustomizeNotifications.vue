        ...
        <el-form-item :label="$root.labels.notification_template" prop="notificationTemplate">
          <el-select v-model="testNotification.notificationTemplate">
            <el-option
                ...
                :disabled="$root.isLite && !['customer_appointment_approved', 'provider_appointment_approved', 'customer_appointment_pending', 'provider_appointment_pending',
                'customer_event_approved', 'provider_event_approved', 'customer_appointment_canceled', 'customer_appointment_rejected', 'provider_appointment_canceled', 'provider_appointment_rejected'].includes(notification.name)"
            >
            </el-option>
          </el-select>
        </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      isNotificationVisible (item) {
        ...
        if ((item.name === 'customer_cart' || item.name === 'provider_cart') && !(this.$root.licence.isPro || this.$root.licence.isDeveloper)) {
          return false
        }
      ...
      isDisabled: function (type, item) {
        item.status = (item.name !== (type + '_appointment_approved') && item.name !== (type + '_appointment_pending') && item.name !== (type + '_appointment_canceled') && item.name !== (type + '_appointment_rejected') && item.name !== (type + '_event_approved')) ? 'disabled' : item.status

        return this.$root.isLite ? (item.name !== (type + '_appointment_approved') && item.name !== (type + '_appointment_pending') && item.name !== (type + '_appointment_canceled') && item.name !== (type + '_appointment_rejected') && item.name !== (type + '_event_approved')) : false
      },
